"""
HuggingFace LLM client for user_stories_agent.
...
if HF_TOKEN is None:
    for key, val in os.environ.items():
        if key.endswith("HF_TOKEN") and key != "HF_TOKEN":
            HF_TOKEN = val
            print("[user_stories_agent] Recovered HF_TOKEN from BOM-corrupted env var", file=sys.stderr)
            break
...
if not HF_TOKEN:
    print("[user_stories_agent] WARNING: HF_TOKEN not found in .env", file=sys.stderr)
...
def call_inference(prompt: str, model: str = None, token: Optional[str] = None,
    ...
    for m in models_to_try:
        ...
        print(f"[user_stories_agent] Trying model: {m}...", file=sys.stderr)
        ...
        for attempt in range(2):
            try:
                ...
                if text and text.strip():
                    print(f"[user_stories_agent] Success with model: {m}", file=sys.stderr)
                    return text.strip()

                print(f"[user_stories_agent] {m}: empty response", file=sys.stderr)
                break
                ...
                if "sufficient permissions" in err or "Inference Providers" in err:
                    ...
                    print(f"[user_stories_agent] {m}: access denied – skipping", file=sys.stderr)
                    break
                elif "404" in err or "not found" in err.lower():
                    _model_unavailable.add(m)
                    print(f"[user_stories_agent] {m}: not found – skipping", file=sys.stderr)
                    break
                elif "429" in err or "rate" in err.lower() or "quota" in err.lower():
                    print(f"[user_stories_agent] {m}: rate limited", file=sys.stderr)
                    if attempt == 0:
                        time.sleep(5)
                elif "503" in err or "loading" in err.lower():
                    print(f"[user_stories_agent] {m}: model loading", file=sys.stderr)
                    if attempt == 0:
                        time.sleep(10)
                else:
                    print(f"[user_stories_agent] {m} error: {err[:300]}", file=sys.stderr)
                    if attempt == 0:
                        time.sleep(3)
