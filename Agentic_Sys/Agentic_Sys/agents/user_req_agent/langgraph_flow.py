...
class FlowController:
    ...
    def run(self, extracted_text: str) -> Tuple[RequirementsArtifact, dict]:
        attempts = 0
        start = time.time()
        ...
        while attempts < self.max_retries:
            attempts += 1
            prompt = build_prompt(extracted_text, last_validation_errors=last_errors)
            ...
            try:
                artifact = RequirementsArtifact.model_validate(parsed)
